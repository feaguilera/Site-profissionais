    ...
    <div className="min-h-screen bg-[#0A0A0A] text-white">
      ...
      <nav className={`fixed w-full z-[100] transition-all duration-300 ${scrolled ? 'bg-black/90 backdrop-blur-md py-3' : 'bg-transparent py-6'}`}>
        <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
          <div className="flex items-center gap-2 cursor-pointer" onClick={() => window.scrollTo({top:0, behavior:'smooth'})}>
            <div className="w-10 h-10 bg-[#C6FF00] rounded-lg flex items-center justify-center text-black">
               <Zap size={24} fill="currentColor" />
            </div>
            <span className="text-2xl font-impact tracking-tighter uppercase italic">TITAN<span className="text-[#C6FF00]">PERFORMANCE</span></span>
          </div>
          ...
          <div className="hidden lg:flex items-center space-x-12">
            {['Início', 'Planos', 'Metodologia', 'FAQ'].map((item) => (
              <button
                ...
                className="text-[11px] font-bold uppercase tracking-widest hover:text-[#C6FF00] transition-colors"
              >
                {item}
              </button>
            ))}
            <button onClick={openWhatsApp} className="px-6 py-3 btn-neon text-[10px]">
              QUERO TREINAR
            </button>
          </div>

          <button className="lg:hidden text-[#C6FF00]" onClick={() => setIsMenuOpen(true)}>
            <Menu size={28} />
          </button>
        </div>
      </nav>
      ...
      <div className={`fixed inset-0 z-[200] bg-black transition-all duration-500 ${isMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="p-8 flex justify-between items-center border-b border-white/10">
           ...
           <button onClick={() => setIsMenuOpen(false)} className="text-[#C6FF00]"><X size={32} /></button>
        </div>
        <div className="flex flex-col p-12 gap-8 text-center">
           {['Início', 'Planos', 'Metodologia', 'FAQ'].map((item) => (
             <button
               key={item}
               onClick={() => scrollToSection(item.toLowerCase())}
               className="text-4xl font-impact uppercase italic text-white hover:text-[#C6FF00]"
             >
               {item}
             </button>
           ))}
           <button onClick={openWhatsApp} className="mt-8 py-6 btn-neon text-lg">
             COMEÇAR AGORA
           </button>
        </div>
      </div>
      ...
      <section id="inicio" className="relative min-h-screen flex items-center pt-20 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10 w-full">
           <div className="max-w-3xl">
              <div className="inline-flex items-center gap-2 px-3 py-1 border border-[#C6FF00]/30 rounded-full mb-8 bg-[#C6FF00]/10">
                 <span className="w-2 h-2 bg-[#C6FF00] rounded-full animate-pulse"></span>
                 <span className="text-[10px] font-black uppercase tracking-[0.2em] text-[#C6FF00]">Novas Vagas para Personal VIP</span>
              </div>
              <h1 className="text-6xl sm:text-8xl lg:text-[100px] font-impact leading-[0.9] italic uppercase mb-8">
                PARE DE <br/>
                <span className="text-[#C6FF00]">TENTAR.</span> <br/>
                COMECE A <br/>
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                 <button onClick={() => scrollToSection('planos')} className="px-10 py-5 btn-neon text-sm shadow-[0_0_30px_rgba(198,255,0,0.3)]">
        ...
        <div className="absolute bottom-10 right-10 hidden lg:grid grid-cols-2 gap-4">
           {[
             ...
             <div key={i} className="glass-card p-6 rounded-2xl">
                <p className="text-4xl font-impact italic text-[#C6FF00]">{stat.val}</p>
      ...
      {/* Benefits Strip */}
      <div className="bg-[#C6FF00] py-4 overflow-hidden whitespace-nowrap border-y border-black">
         <div className="flex animate-[scroll_20s_linear_infinite] gap-12 items-center">
            {[1,2,3,4,5].map(i => (
              <div key={i} className="flex items-center gap-12">
                <span className="text-black font-impact text-2xl uppercase italic">MUSCULAÇÃO</span>
                <span className="w-2 h-2 bg-black rounded-full"></span>
                <span className="text-black font-impact text-2xl uppercase italic">CROSSFIT</span>
                <span className="w-2 h-2 bg-black rounded-full"></span>
                <span className="text-black font-impact text-2xl uppercase italic">NUTRIÇÃO</span>
                <span className="w-2 h-2 bg-black rounded-full"></span>
                <span className="text-black font-impact text-2xl uppercase italic">PERSONAL VIP</span>
                <span className="w-2 h-2 bg-black rounded-full"></span>
              </div>
            ))}
         </div>
      ...
      <section id="planos" className="py-32 bg-[#0F0F0F]">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-24">
            <h2 className="text-5xl lg:text-7xl font-impact uppercase italic mb-6">ESCOLHA SEU <span className="text-[#C6FF00]">LEVEL.</span></h2>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {SERVICES.map((s, i) => (
              <div key={s.id} className={`relative group p-10 rounded-[32px] transition-all duration-500 ${i === 1 ? 'bg-zinc-900 border-2 border-[#C6FF00] scale-105 z-10' : 'bg-[#1A1A1A] border border-white/5'}`}>
                {i === 1 && (
                  <div className="absolute -top-5 left-1/2 -translate-x-1/2 bg-[#C6FF00] text-black px-4 py-1 text-[10px] font-black uppercase tracking-widest rounded-full">
                    RECOMENDADO
                  </div>
                )}
                <div className="mb-8 text-[#C6FF00]">
                  {s.icon}
                </div>
                ...
                <button 
                  ...
                  className={`w-full py-5 rounded-xl font-black text-xs uppercase tracking-widest transition-all ${i === 1 ? 'bg-[#C6FF00] text-black hover:shadow-[0_0_30px_rgba(198,255,0,0.5)]' : 'bg-white/5 text-white border border-white/10 hover:bg-white/10'}`}
                >
                  SELECIONAR PLANO
                </button>
              </div>
      ...
      <section id="metodologia" className="py-32 relative overflow-hidden">
        <div className="max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-24 items-center">
           <div>
              <span className="text-[#C6FF00] font-black text-[10px] uppercase tracking-[0.4em] mb-4 block">Porque somos diferentes</span>
              ...
              <div className="space-y-12">
                 {BENEFITS.map((b, i) => (
                   <div key={i} className="flex gap-6 group">
                      <div className="w-16 h-16 bg-[#C6FF00]/10 text-[#C6FF00] rounded-2xl flex items-center justify-center flex-shrink-0 group-hover:bg-[#C6FF00] group-hover:text-black transition-all duration-300">
                         {b.icon}
                      </div>
                      <div>
           ...
           <div className="relative">
              ...
              {/* Badge Overlay */}
              <div className="absolute -bottom-10 -right-10 bg-[#C6FF00] p-10 rounded-full text-black shadow-2xl animate-float hidden sm:block">
      ...
      <section id="faq" className="py-32 bg-[#0A0A0A]">
        <div className="max-w-3xl mx-auto px-6">
           <div className="text-center mb-16">
              <h2 className="text-4xl font-impact uppercase italic">DÚVIDAS <span className="text-[#C6FF00]">FREQUENTES</span></h2>
           </div>
              ...
              {FAQ.map((item, i) => (
                <div key={i} className="border border-white/5 rounded-2xl overflow-hidden">
                   ...
                   >
                      ...
                      <ChevronDown className={`text-[#C6FF00] transition-transform ${openFaq === i ? 'rotate-180' : ''}`} />
                   </button>
      ...
      <footer className="py-20 bg-black border-t border-white/5">
        <div className="max-w-7xl mx-auto px-6 text-center">
           <div className="flex items-center justify-center gap-2 mb-12">
              <Zap size={32} className="text-[#C6FF00]" fill="currentColor" />
           ...
           <div className="flex flex-wrap justify-center gap-8 mb-16 text-zinc-500 font-bold text-[10px] uppercase tracking-[0.2em]">
              {['Planos', 'Metodologia', 'Horários', 'Localização'].map(link => (
                <button key={link} className="hover:text-[#C6FF00] transition-colors">{link}</button>
              ))}
           </div>

           <div className="flex justify-center gap-6 mb-16">
              <a href="#" className="w-12 h-12 rounded-full border border-white/10 flex items-center justify-center hover:bg-[#C6FF00] hover:text-black transition-all">
                 <Instagram size={20} />
              </a>
              <button onClick={openWhatsApp} className="w-12 h-12 rounded-full border border-white/10 flex items-center justify-center hover:bg-[#C6FF00] hover:text-black transition-all">
      ...
      <div className="lg:hidden fixed bottom-8 left-6 right-6 z-[150]">
         <button onClick={openWhatsApp} className="w-full py-5 btn-neon flex items-center justify-center gap-4 text-sm shadow-[0_0_30px_rgba(198,255,0,0.4)]">
