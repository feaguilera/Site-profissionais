...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen selection:bg-[#D4FF00] selection:text-black">
      {/* Navbar */}
      <nav className={`fixed w-full z-[100] transition-all duration-300 ${scrolled ? 'bg-[#0A0A0A]/95 backdrop-blur-md shadow-2xl py-3 border-b border-white/5' : 'bg-transparent py-8'}`}>
        <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
          <div className="flex items-center gap-3 cursor-pointer" onClick={() => scrollToSection('home')}>
            <div className="bg-[#D4FF00] p-2 rounded-sm rotate-3">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-[10px] font-bold text-[#D4FF00] uppercase tracking-[0.2em]">Performance Hub</span>
            </div>
          </div>
          ...
          <div className="hidden lg:flex items-center space-x-10">
            {['Serviços', 'Pipeline', 'Reviews', 'FAQ'].map((item) => (
              <button
                ...
                className="text-[11px] font-extrabold tracking-widest uppercase text-slate-400 hover:text-[#D4FF00] transition-all"
              >
                {item}
              </button>
            ))}
            <button 
              ...
              className="bg-[#D4FF00] text-black px-8 py-3 rounded-none font-black text-[10px] uppercase tracking-[0.2em] btn-performance"
            >
              Consultar Setup
            </button>
          </div>
        ...
        {isMenuOpen && (
          <div className="lg:hidden fixed inset-0 bg-[#0A0A0A] z-50 flex flex-col p-8 animate-in slide-in-from-right">
            <div className="flex justify-between items-center mb-16">
              <span className="text-2xl font-heading font-black italic text-[#D4FF00]">CARBON</span>
            ...
            <div className="flex flex-col gap-6">
              {['Serviços', 'Pipeline', 'Reviews', 'FAQ'].map((item) => (
                <button
                  key={item}
                  onClick={() => scrollToSection(item.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, ""))}
                  className="text-3xl font-black text-left border-l-4 border-white/5 pl-6 text-white hover:border-[#D4FF00]"
                >
                  {item}
                </button>
              ))}
              <button 
                onClick={openWhatsApp}
                className="bg-[#D4FF00] text-black py-6 rounded-none font-black mt-6 uppercase tracking-widest text-sm flex justify-center items-center gap-3 shadow-xl"
              >
                Orçamento WhatsApp
              </button>
            </div>
      ...
      <section id="home" className="relative min-h-screen flex items-center pt-20 overflow-hidden bg-[#0A0A0A]">
        {/* Background Accents with Zero Z-Index */}
        <div className="absolute top-0 right-0 w-1/3 h-full bg-[#D4FF00]/5 -skew-x-12 translate-x-1/4 pointer-events-none z-0"></div>
        <div className="absolute -bottom-24 -left-24 w-96 h-96 bg-[#D4FF00]/10 rounded-full blur-[100px] pointer-events-none z-0"></div>
        
        <div className="max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-16 items-center relative z-20">
          {/* Text Container with MAX Z-Index to prevent hiding behind image */}
          <div className="z-50 order-2 lg:order-1 relative">
            <div className="inline-flex items-center gap-2 mb-6 bg-white/5 px-4 py-1.5 rounded-none border-l-4 border-[#D4FF00]">
            ...
            <h1 className="text-5xl md:text-8xl font-heading font-black text-white mb-8 leading-[0.9] uppercase italic tracking-tighter break-words relative z-50">
              Performance <br />
              <span className="text-[#D4FF00] relative block lg:inline z-50">
                Sem Limites.
                <div className="absolute -bottom-2 left-0 w-full h-2 bg-[#D4FF00]/30 -z-10"></div>
              </span>
            </h1>
            ...
            <div className="flex flex-col sm:flex-row gap-6">
              <button 
                onClick={openWhatsApp}
                className="bg-[#D4FF00] text-black px-12 py-6 rounded-none font-black text-xs tracking-[0.2em] uppercase btn-performance flex items-center justify-center gap-3 shadow-2xl"
              >
              ...
              <div className="flex flex-col justify-center">
                <div className="flex items-center gap-2 text-white font-black text-sm uppercase italic">
                  <Activity size={16} className="text-[#D4FF00]" /> +45% HP Real
                </div>
          ...
          <div className="relative hidden lg:block z-10 order-1 lg:order-2">
            <div className="relative p-4 border border-white/5 bg-[#111111] z-10">
              <img 
              ...
              />
              <div className="absolute bottom-10 left-10 right-10 p-8 bg-[#0A0A0A]/90 border-l-8 border-[#D4FF00] backdrop-blur-md z-20">
                <div className="flex justify-between items-end">
                   <div>
                     <p className="text-[9px] text-[#D4FF00] font-black uppercase tracking-widest mb-1">Status do Setup</p>
      ...
      {/* Brands Bar */}
      <div className="bg-[#050505] py-12 border-y border-white/5 relative z-30">
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid md:grid-cols-4 gap-12">
            {BENEFITS.map((benefit, idx) => (
              <div key={idx} className="group border-t border-white/5 pt-12 hover:border-[#D4FF00] transition-all">
                <div className="text-[#D4FF00] mb-8 group-hover:scale-110 transition-transform">
                  {benefit.icon}
                </div>
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex flex-col md:flex-row justify-between items-end mb-24 gap-8">
            <div className="max-w-2xl">
              <span className="text-[#D4FF00] text-[10px] font-black tracking-[0.4em] uppercase mb-4 block">Engineered Excellence</span>
              <h2 className="text-5xl font-heading font-black text-white italic uppercase leading-none">Power & <br />Precision Units.</h2>
            </div>
            <button className="text-white/40 hover:text-[#D4FF00] font-black text-[10px] uppercase tracking-widest transition-all">Ver todos os estágios</button>
          </div>

          <div className="grid md:grid-cols-3 gap-8">
            {SERVICES.map((s) => (
              <div key={s.id} className="bg-[#111111] p-10 border-l-2 border-white/5 hover:border-[#D4FF00] transition-all group relative overflow-hidden">
                <div className="absolute -top-10 -right-10 w-40 h-40 bg-[#D4FF00]/5 rounded-full blur-3xl group-hover:bg-[#D4FF00]/10 transition-all"></div>
                <div className="flex justify-between items-start mb-12">
                  <div className="text-[#D4FF00] group-hover:animate-pulse">
                    {s.icon}
                  </div>
                  {s.tag && <span className="text-[9px] font-black uppercase text-[#D4FF00] tracking-widest px-3 py-1 bg-[#D4FF00]/10">{s.tag}</span>}
                </div>
                ...
                <div className="flex items-center justify-between pt-8 border-t border-white/5">
                  ...
                  <button onClick={openWhatsApp} className="bg-[#D4FF00] p-3 text-black group-hover:px-6 transition-all">
                    <ArrowRight size={18} />
                  </button>
                </div>
              </div>
      ...
      <section id="pipeline" className="py-32 bg-[#0A0A0A]">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-24">
            ...
            <div className="w-24 h-1 bg-[#D4FF00] mx-auto mt-6"></div>
          </div>
          <div className="grid md:grid-cols-4 gap-8">
            {PROCESS.map((p, idx) => (
              <div key={idx} className="relative p-8 bg-[#111111] group">
                <div className="absolute top-0 left-0 w-1 h-0 group-hover:h-full bg-[#D4FF00] transition-all duration-500"></div>
              ...
              <div key={idx} className="bg-[#0A0A0A] p-12 border border-white/5 relative">
                ...
                <div className="flex items-center gap-5 pt-8 border-t border-white/5">
                  ...
                  <div>
                    ...
                    <p className="text-[#D4FF00] text-[9px] font-black uppercase tracking-widest mt-1">{t.role}</p>
                  </div>
                </div>
              </div>
            ))}
      ...
      <section id="faq" className="py-32 bg-[#0A0A0A]">
        <div className="max-w-3xl mx-auto px-6">
          ...
          <div className="space-y-4">
            {FAQ.map((item, idx) => (
              <div key={idx} className="bg-[#111111] border-l-4 border-white/5 hover:border-[#D4FF00] transition-all">
                <button 
                ...
                >
                  ...
                  <ChevronDown className={`text-[#D4FF00] transition-transform ${activeFaq === idx ? 'rotate-180' : ''}`} size={20} />
                </button>
      ...
      <section className="py-48 bg-[#0A0A0A] relative overflow-hidden">
        <div className="absolute inset-0 opacity-5 flex items-center justify-center pointer-events-none">
          <Gauge size={800} className="text-[#D4FF00]" />
        </div>
        <div className="max-w-4xl mx-auto px-6 relative z-50 text-center">
          <h2 className="text-7xl md:text-9xl font-heading font-black text-white mb-16 leading-none uppercase italic tracking-tighter">Peak <br /> <span className="text-[#D4FF00]">Power.</span></h2>
          <div className="flex flex-col sm:flex-row gap-6 justify-center relative z-50">
            <button 
              onClick={openWhatsApp}
              className="bg-[#D4FF00] text-black px-16 py-7 rounded-none font-black text-sm tracking-[0.3em] uppercase btn-performance shadow-2xl flex items-center justify-center gap-4"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-20 mb-32">
            <div className="col-span-1 md:col-span-1">
              <div className="flex items-center gap-3 mb-10">
                <div className="bg-[#D4FF00] p-2 rotate-3">
              ...
              <div className="flex gap-6">
                <a href="#" className="text-white/20 hover:text-[#D4FF00] transition-all"><Instagram size={24} /></a>
                <a href="#" className="text-white/20 hover:text-[#D4FF00] transition-all"><MessageCircle size={24} /></a>
              </div>
            </div>
            ...
            {[
              ...
              <div key={idx}>
                <h4 className="text-white mb-10 text-[11px] font-black uppercase tracking-[0.4em] border-l-2 border-[#D4FF00] pl-4">{col.title}</h4>
      ...
      <button 
        onClick={openWhatsApp}
        className="fixed bottom-10 right-10 bg-[#D4FF00] text-black p-5 rounded-none shadow-2xl hover:scale-110 active:scale-95 transition-all z-[100] group flex items-center gap-4"
      >
